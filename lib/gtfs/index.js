...
// Obtener información de todas las líneas
const gtfsGetLineas = async () => {
  const gtfsRoutes = await gtfs.getRoutes();

  const lineas = await Promise.all(
    gtfsRoutes.map(async (route) => {
      ...
      return {
        ...
        descripcion: route.route_desc || '',
        tipo: route.route_type,
        url: route.route_url || '',
        color: route.route_color ? `#${route.route_color}` : '',
        colorTexto: route.route_text_color ? `#${route.route_text_color}` : '',
        agencia: route.agency_id,
...
// Obtener información de una línea específica
const gtfsGetLinea = async (routeShortName) => {
  const gtfsRoutes = await gtfs.getRoutes({ route_short_name: routeShortName });

  if (!gtfsRoutes || gtfsRoutes.length === 0) {
    return { error: `No existe la línea ${routeShortName}` };
  }

  const route = gtfsRoutes[0];
  const trips = await gtfs.getTrips({ route_id: route.route_id });
  
  // Encontrar el trip con más paradas (más representativo del recorrido completo)
  let representativeTrip = null;
  let maxStops = 0;
  
  for (const trip of trips) {
    const tripStopTimes = await gtfs.getStoptimes({ trip_id: trip.trip_id });
    if (tripStopTimes.length > maxStops) {
      maxStops = tripStopTimes.length;
      representativeTrip = trip;
    }
  }

  // Obtener los stop_times del trip representativo
  const representativeStopTimes = representativeTrip 
    ? await gtfs.getStoptimes({ trip_id: representativeTrip.trip_id })
    : [];

  // Crear un mapa de stop_id a secuencia usando el trip representativo
  const stopSequenceMap = new Map();
  representativeStopTimes.forEach((stopTime) => {
    stopSequenceMap.set(stopTime.stop_id, stopTime.stop_sequence);
  });

  const stops = await gtfs.getStops({ route_id: route.route_id });

  // Obtener todos los stop_times para información adicional
  const allStopTimes = [];
  for (const trip of trips) {
    const tripStopTimes = await gtfs.getStoptimes({ trip_id: trip.trip_id });
  ...
  // Obtener información de paradas con secuencia
  const stopsWithSequence = stops.map((stop) => {
    const sequence = stopSequenceMap.get(stop.stop_id) || 999999;
    const stopTimes = allStopTimes.filter((st) => st.stop_id === stop.stop_id);
    const sequences = [...new Set(stopTimes.map((st) => st.stop_sequence))].sort((a, b) => a - b);

    return {
      id: stop.stop_id,
      codigo: stop.stop_code,
      nombre: stop.stop_name,
      latitud: stop.stop_lat,
      longitud: stop.stop_lon,
      url: stop.stop_url || '',
      secuencias: sequences,
      _sequence: sequence, // Campo interno para ordenar
    };
  });

  // Ordenar paradas según el trip representativo
  stopsWithSequence.sort((a, b) => a._sequence - b._sequence);

  // Remover el campo interno de ordenamiento
  stopsWithSequence.forEach((stop) => delete stop._sequence);
