...
const {
  ...
  GTFS_REFRESH_JITTER,
  GTFS_RT_RETRY_DELAY,
  GTFS_STATIC_REFRESH_RATE,
  GTFS_RT_CACHE,
} = environment;
const gtfsConfig = require('./config');
const {
  updateRealtimeWithQualityRetry,
} = require('./realtime-quality');
const { createRealtimeScheduler } = require('./realtime-scheduler');
const moment = require('moment-timezone');
const { importGtfs } = require('./helpers/import');
const { updateGtfsHealth } = require('../../routes/health');

let gtfs;
let staticUpdateInProgress = false;
const counters = {
  changes: 0,
  total: 0,
};
const configuredRealtimeRetryDelay = Number.parseInt(
  GTFS_RT_RETRY_DELAY,
  10,
);
const realtimeRetryDelay =
  Number.isInteger(configuredRealtimeRetryDelay) &&
  configuredRealtimeRetryDelay > 0
    ? configuredRealtimeRetryDelay
    : 7000;
  ...
  return new Promise((resolve) => {
    import('gtfs').then(async (module) => {
      ...
      async function updateStaticGtfs() {
        if (staticUpdateInProgress) {
          console.warn('⏭️ Actualización GTFS estática omitida: ya hay una en curso');
          return;
        }

        staticUpdateInProgress = true;
        try {
          console.log('🔄 Actualizando GTFS estático (descarga + reimportación SQLite)...');
          await importGtfs();
          invalidateActiveServiceIdsCache();
          console.log('✅ GTFS estático actualizado y reimportado correctamente');
        } catch (error) {
          console.error('Error al importar estáticos importGtfs():', error);
        } finally {
          staticUpdateInProgress = false;
        }
      }
      ...
      // Repetir el ciclo completo, incluyendo la reimportación de SQLite.
      // Se programa al terminar cada intento para evitar solapamientos y para
      // que un error no impida el siguiente reintento.
      const scheduleStaticUpdate = () => {
        setTimeout(async () => {
          await updateStaticGtfs();
          scheduleStaticUpdate();
        }, GTFS_STATIC_REFRESH_RATE || 43200000);
      };
      scheduleStaticUpdate();
      ...
      // Programar cada actualización al terminar la anterior y añadir jitter
      // para no sincronizarnos con la ventana de regeneración del proveedor.
      const realtimeScheduler = createRealtimeScheduler({
        update: updateGtfsRt,
        refreshRate: GTFS_REFRESH_RATE,
        refreshJitter: GTFS_REFRESH_JITTER,
        onError: (error) => {
          console.error('Error inesperado en el scheduler GTFS realtime:', error);
        },
      });
...
const updateGtfsRt = async () => {
  try {
    ...
    const { quality: importQuality } = await updateRealtimeWithQualityRetry({
      importRealtime: () => gtfs.updateGtfsRealtime(gtfsConfig),
      getTripUpdates: () => gtfs.getTripUpdates(),
      getVehiclePositions: () => gtfs.getVehiclePositions(),
      retryDelay: realtimeRetryDelay,
      onPartial: (quality) => {
        console.warn(
          `⚠️ Lote GTFS-RT parcial: ${quality.tripUpdatesCount} viajes y ${quality.vehiclePositionsCount} vehículos. Reintentando en ${realtimeRetryDelay} ms...`,
        );
      },
    });

    if (importQuality.suspicious) {
      throw new Error(
        `GTFS-RT trip updates incompleto tras reintento: ${importQuality.tripUpdatesCount} viajes para ${importQuality.vehiclePositionsCount} vehículos`,
      );
    }
...
// Los horarios estáticos pueden cambiar durante el mismo día (por ejemplo,
// al entrar en vigor el horario de verano). En ese caso la caché diaria deja
// de ser válida aunque la fecha no haya cambiado.
const invalidateActiveServiceIdsCache = () => {
...
const ALERT_ENTITY_METADATA_FIELDS = new Set([
  'alert_id',
  'created_timestamp',
...
const getAlertInformedEntities = (alert) => {
  if (
    Array.isArray(alert.informed_entity) &&
  ...
  if (
    ...
    return alert.informed_entities;
  }

  return [];
};

const getAlertEntitySelector = (entity) => {
  ...
  return Object.fromEntries(
    Object.entries(entity)
      .filter(([key]) => !ALERT_ENTITY_METADATA_FIELDS.has(key))
      .sort(([keyA], [keyB]) => keyA.localeCompare(keyB)),
  );
};

const canonicalizeAlertValue = (value) => {
  if (Array.isArray(value)) {
    return value
      .map(canonicalizeAlertValue)
      .sort((valueA, valueB) =>
  ...
  if (value && typeof value === 'object') {
    return Object.fromEntries(
      Object.entries(value)
        ...
        .map(([key, nestedValue]) => [
          key,
          canonicalizeAlertValue(nestedValue),
        ]),
    );
  }
...
const serializeAlertKey = (value) =>
  JSON.stringify(canonicalizeAlertValue(value));

const parseActivePeriod = (activePeriod) => {
  if (typeof activePeriod !== 'string') {
    return activePeriod ?? null;
  }

  try {
    return JSON.parse(activePeriod);
  } catch {
    return activePeriod;
  }
};
...
const getAlertContent = (alert) => ({
  ...
  activePeriod: alert.active_period ?? null,
  startTime: alert.start_time ?? null,
  endTime: alert.end_time ?? null,
  headerText: alert.header_text ?? alert.headline ?? null,
  descriptionText: alert.description_text ?? alert.description ?? null,
});

const getUniqueAlertEntities = (entities) => {
  const seen = new Set();

  return entities.filter((entity) => {
    const key = serializeAlertKey(getAlertEntitySelector(entity));
    if (seen.has(key)) {
      return false;
    }
...
const formatServiceAlerts = (serviceAlerts, lineas, paradas) => {
  const routesById = new Map(
    lineas.map((linea) => [normalizeAlertId(linea.route_id), linea]),
  );
  const stopsById = new Map(
    paradas.map((parada) => [normalizeAlertId(parada.stop_id), parada]),
  );
  const stopsByCode = new Map(
    paradas.map((parada) => [normalizeAlertId(parada.stop_code), parada]),
  );
  ...
  for (const alert of serviceAlerts) {
    ...
    const alertId = normalizeAlertId(alert.id ?? alert.alert_id);
    const informedEntities = getUniqueAlertEntities(
      getAlertInformedEntities(alert),
    );
    const entitiesForOutput = informedEntities.length
      ? informedEntities
      : [null];
    const informedEntitiesKey = informedEntities.map(getAlertEntitySelector);

    for (const entity of entitiesForOutput) {
      const entitySelector = getAlertEntitySelector(entity);
      const deduplicationKey = serializeAlertKey({
        content: {
          cause: content.cause,
          effect: content.effect,
          header_text: content.headerText,
          description_text: content.descriptionText,
          url: alert.url ?? null,
          tts_header_text: alert.tts_header_text ?? null,
          tts_description_text: alert.tts_description_text ?? null,
          severity_level: alert.severity_level ?? null,
        },
        informed_entities: informedEntitiesKey,
        active_period: parseActivePeriod(content.activePeriod),
        start_time: content.startTime,
        end_time: content.endTime,
        entity: entitySelector,
      });

      if (seenAlerts.has(deduplicationKey)) {
        continue;
      }
      seenAlerts.add(deduplicationKey);

      const routeId = normalizeAlertId(entity?.route_id);
      const stopId = normalizeAlertId(entity?.stop_id);
      const routeInfo = routeId ? routesById.get(routeId) : null;
      const stopInfo = stopId
        ? stopsById.get(stopId) || stopsByCode.get(stopId)
        : null;
      ...
      formattedAlerts.push({
        ...
        cause: content.cause,
        effect: alert.effect ?? null,
        active_period: content.activePeriod,
        url: alert.url ?? null,
        header_text: content.headerText,
        description_text: content.descriptionText,
        ruta: {
          causa: content.cause,
          gtfsRouteId: routeId,
          linea: routeInfo?.route_short_name ?? null,
          parada: stopInfo?.stop_code ?? null,
          start_time: content.startTime,
          end_time: content.endTime,
          active_period: content.activePeriod,
          is_update: alert.is_updated ?? null,
        },
        resumen: content.headerText,
        descripcion: content.descriptionText,
      });
    }
  }
...
const gtfsGetAlerts = async (gtfsClient = gtfs) => {
  try {
    if (!gtfsClient) {
      return [];
    }

    const [serviceAlerts, lineas, paradas] = await Promise.all([
      Promise.resolve(gtfsClient.getServiceAlerts()),
      Promise.resolve(gtfsClient.getRoutes()),
      Promise.resolve(gtfsClient.getStops()),
    ]);

    return formatServiceAlerts(
